...
if [ -d "$VOLTDB_HOME/lib/voltdb" ]; then
    # Debian paths
    export CLASSPATH="`ls -1 $VOLTDB_HOME/lib/voltdb/voltdb-*.jar $VOLTDB_HOME/lib/*.jar lib/*.jar | tr '\n' ':'`"
    export LICENSE="$VOLTDB_HOME/lib/voltdb/license.xml"
    export LOG4J="$VOLTDB_HOME/lib/voltdb/log4j.xml"
else
    # non-Debian paths
    export CLASSPATH="`ls -1 $VOLTDB_HOME/voltdb/voltdb-*.jar $VOLTDB_HOME/lib/*.jar lib/*.jar | tr '\n' ':'`"
